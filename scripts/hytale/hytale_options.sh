...
if [ -n "${HYTALE_AUTH_MODE:-}" ]; then
    if [ "$HYTALE_AUTH_MODE" = "authenticated" ] || [ "$HYTALE_AUTH_MODE" = "offline" ] || [ "$HYTALE_AUTH_MODE" = "insecure" ]; then
        export HYTALE_AUTH_MODE_OPT="--auth-mode=$HYTALE_AUTH_MODE"
        printf "${GREEN}$HYTALE_AUTH_MODE${NC}\n"
    else
        printf "${RED}invalid: $HYTALE_AUTH_MODE${NC} (use 'authenticated' , 'offline' or 'insecure')${NC}\n"
    fi
else
